...
class MessageConsumer:
    ...
    def start_consumers(self):
        ...
        self._start_consumer("ordonnances.notifications", self._handle_prescription_notification)
    ...
    def _handle_prescription_created(self, channel, method, properties, body):
        ...
        try:
            ...
            logger_service.error(f"Error processing prescription creation: {e!s}")
    ...
    def _handle_prescription_notification(self, channel, method, properties, body):
        ...
        try:
            ...
            logger_service.info(f"Received notification for prescription {prescription_id}: {event}")
            ...
            logger_service.error(f"Error processing prescription notification: {e!s}")
            channel.basic_ack(delivery_tag=method.delivery_tag)
    ...
    def _handle_patient_prescription(self, channel, method, properties, body):
        ...
        try:
            ...
            logger_service.info(f"Processing patient prescription action: {action} for patient {patient_id}")

            # Acknowledge message immediately
            channel.basic_ack(delivery_tag=method.delivery_tag)

            # Process the patient prescription action
            self._process_patient_prescription(prescription_id, patient_id, action, message)
        except Exception as e:
            logger_service.error(f"Error processing patient prescription: {e!s}")
            channel.basic_ack(delivery_tag=method.delivery_tag)
    ...
    def _handle_doctor_prescription(self, channel, method, properties, body):
        ...
        try:
            ...
            logger_service.info(f"Processing doctor prescription action: {action} for doctor {doctor_id}")

            # Acknowledge message immediately
            channel.basic_ack(delivery_tag=method.delivery_tag)

            # Process the doctor prescription action
            self._process_doctor_prescription(prescription_id, doctor_id, action, message)
        except Exception as e:
            logger_service.error(f"Error processing doctor prescription: {e!s}")
            channel.basic_ack(delivery_tag=method.delivery_tag)

    def _process_prescription_creation(self, prescription_id: str, message: dict[str, Any]):
        ...
        try:
            ...
            prescription = self.mongodb_client.db.ordonnances.find_one({"_id": prescription_id})

            if not prescription:
                # Prescription doesn't exist, store it
                logger_service.info(f"New prescription {prescription_id} received, storing it")
                ...
                logger_service.info(f"Prescription {prescription_id} already exists, updating")
            ...
            logger_service.info(f"Successfully processed prescription {prescription_id}")

        except Exception as e:
            logger_service.error(f"Error in _process_prescription_creation: {e!s}")

    def _process_prescription_notification(self, prescription_id: str, event: str, message: dict[str, Any]):
        ...
        try:
            ...
            logger_service.info(f"Processed notification for prescription {prescription_id}: {event}")
    ...
    def _process_patient_prescription(
        ...
        try:
            ...
            if action == "viewed":
                ...
                doctor_id = message.get("doctor_id")
                if doctor_id:
                    self.rabbitmq_client.notify_doctor_prescription(prescription_id, doctor_id, "renewal_requested")

            logger_service.info(f"Processed patient action {action} for prescription {prescription_id}")

        except Exception as e:
            logger_service.error(f"Error processing patient prescription action: {e!s}")

    def _process_doctor_prescription(self, prescription_id: str, doctor_id: str, action: str, message: dict[str, Any]):
        """Process a doctor prescription action"""
        try:
            # Handle different doctor actions
            if action == "renewed":
                # Create new prescription based on renewal
                new_prescription_id = f"renewed-{prescription_id}-{int(datetime.utcnow().timestamp())}"

                # Copy original prescription data with updates
                original = self.mongodb_client.db.ordonnances.find_one({"_id": prescription_id})
                if original:
                    ...
                    new_prescription = {
                        ...
                        "updated_at": datetime.utcnow(),
                        "status": "created",
                        "medications": message.get("medications", original.get("medications", [])),
                        "instructions": message.get("instructions", original.get("instructions")),
                    ...
                    logger_service.warning(f"Cannot renew prescription {prescription_id} - original not found")
                ...
                patient_id = message.get("patient_id")
                if patient_id:
                    self.rabbitmq_client.notify_patient_prescription(prescription_id, patient_id, "cancelled")

            logger_service.info(f"Processed doctor action {action} for prescription {prescription_id}")

        except Exception as e:
            logger_service.error(f"Error processing doctor prescription action: {e!s}")

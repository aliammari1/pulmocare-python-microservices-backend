...
meter = metrics.get_meter("ordonnances_service")

# RabbitMQ metrics
RABBITMQ_MESSAGES_PUBLISHED = meter.create_counter(
    name="ordonnances_service_rabbitmq_messages_published_total",
    description="Total number of messages published by ordonnances service",
)

RABBITMQ_PUBLISH_LATENCY = meter.create_histogram(
    name="ordonnances_service_rabbitmq_publish_latency_seconds",
    description="Message publish latency in seconds for ordonnances service",
)

RABBITMQ_CONSUME_LATENCY = meter.create_histogram(
    name="ordonnances_service_rabbitmq_consume_latency_seconds",
    description="Message consume latency in seconds for ordonnances service",
)

RABBITMQ_QUEUE_SIZE = meter.create_up_down_counter(
    name="ordonnances_service_rabbitmq_queue_size",
    description="Number of messages in queue for ordonnances service",
)

RABBITMQ_CONSUMER_COUNT = meter.create_up_down_counter(
    name="ordonnances_service_rabbitmq_consumers",
    description="Number of consumers for ordonnances service",
)

# Circuit breaker metrics
CIRCUIT_BREAKER_STATE = meter.create_up_down_counter(
    name="ordonnances_service_circuit_breaker_state",
    description="Circuit breaker state (0=closed, 1=open, 2=half-open) for ordonnances service",
)

CIRCUIT_BREAKER_FAILURES = meter.create_counter(
    name="ordonnances_service_circuit_breaker_failures_total",
    description="Number of circuit breaker failures for ordonnances service",
)

CIRCUIT_BREAKER_SUCCESS = meter.create_counter(
    name="ordonnances_service_circuit_breaker_success_total",
    description="Number of circuit breaker successes for ordonnances service",
)

CIRCUIT_BREAKER_REJECTED = meter.create_counter(
    name="ordonnances_service_circuit_breaker_rejected_total",
    description="Number of requests rejected due to open circuit for ordonnances service",
)

# Cache metrics
CACHE_HITS = meter.create_counter(
    name="ordonnances_service_cache_hits_total",
    description="Number of cache hits for ordonnances service",
)

CACHE_MISSES = meter.create_counter(
    name="ordonnances_service_cache_misses_total",
    description="Number of cache misses for ordonnances service",
)

# Service dependency metrics
SERVICE_DEPENDENCY_UP = meter.create_up_down_counter(
    name="ordonnances_service_dependency_up",
    description="Whether a service dependency is available (1=up, 0=down) for ordonnances service",
)

SERVICE_DEPENDENCY_LATENCY = meter.create_histogram(
    name="ordonnances_service_dependency_latency_seconds",
    description="Service dependency request latency in seconds for ordonnances service",
)

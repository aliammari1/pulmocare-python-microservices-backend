...
meter = metrics.get_meter("medecins_service")

# RabbitMQ metrics
RABBITMQ_MESSAGES_PUBLISHED = meter.create_counter(
    name="medecins_service_rabbitmq_messages_published_total",
    description="Total number of messages published by medecins service",
)

RABBITMQ_PUBLISH_LATENCY = meter.create_histogram(
    name="medecins_service_rabbitmq_publish_latency_seconds",
    description="Message publish latency in seconds for medecins service",
)

RABBITMQ_CONSUME_LATENCY = meter.create_histogram(
    name="medecins_service_rabbitmq_consume_latency_seconds",
    description="Message consume latency in seconds for medecins service",
)

RABBITMQ_QUEUE_SIZE = meter.create_up_down_counter(
    name="medecins_service_rabbitmq_queue_size",
    description="Number of messages in queue for medecins service",
)

RABBITMQ_CONSUMER_COUNT = meter.create_up_down_counter(
    name="medecins_service_rabbitmq_consumers",
    description="Number of consumers for medecins service",
)

# Circuit breaker metrics
CIRCUIT_BREAKER_STATE = meter.create_up_down_counter(
    name="medecins_service_circuit_breaker_state",
    description="Circuit breaker state (0=closed, 1=open, 2=half-open) for medecins service",
)

CIRCUIT_BREAKER_FAILURES = meter.create_counter(
    name="medecins_service_circuit_breaker_failures_total",
    description="Number of circuit breaker failures for medecins service",
)

CIRCUIT_BREAKER_SUCCESS = meter.create_counter(
    name="medecins_service_circuit_breaker_success_total",
    description="Number of circuit breaker successes for medecins service",
)

CIRCUIT_BREAKER_REJECTED = meter.create_counter(
    name="medecins_service_circuit_breaker_rejected_total",
    description="Number of requests rejected due to open circuit for medecins service",
)

# Cache metrics
CACHE_HITS = meter.create_counter(
    name="medecins_service_cache_hits_total",
    description="Number of cache hits for medecins service",
)

CACHE_MISSES = meter.create_counter(
    name="medecins_service_cache_misses_total",
    description="Number of cache misses for medecins service",
)

CACHE_HIT = meter.create_counter(
    name="medecins_service_cache_hit_total",
    description="Total number of cache hits for medecins service",
)

CACHE_MISS = meter.create_counter(
    name="medecins_service_cache_miss_total",
    description="Total number of cache misses for medecins service",
)

# Service dependency metrics
SERVICE_DEPENDENCY_UP = meter.create_up_down_counter(
    name="medecins_service_dependency_up",
    description="Whether a service dependency is available (1=up, 0=down) for medecins service",
)

SERVICE_DEPENDENCY_LATENCY = meter.create_histogram(
    name="medecins_service_dependency_latency_seconds",
    description="Service dependency request latency in seconds for medecins service",
)
...
def track_cache_metrics(hit: bool, cache_name: str):
    ...
    try:
        if hit:
            ...
            CACHE_HIT.add(1, {"cache_name": cache_name})
        else:
            CACHE_MISSES.add(1, {"cache": cache_name})
            CACHE_MISS.add(1, {"cache_name": cache_name})
    except Exception as e:

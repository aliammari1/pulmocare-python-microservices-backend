...
class PatientService:
    ...
    async def get_patient_by_id(self, patient_id, auth_header=None):
        ...
        try:
            ...
            return await self.circuit_breaker.call(self._get_patient_by_id, patient_id, auth_header)
        except Exception as e:
            logger_service.error(f"Error fetching patient with ID {patient_id}: {e!s}")
            return None
    ...
    async def _get_patient_by_id(self, patient_id, auth_header=None):
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.get(f"{self.base_url}/patients/{patient_id}", headers=headers)

            if response.status_code == 200:
                return response.json()
            else:
                logger_service.error(f"Error fetching patient: HTTP {response.status_code} - {response.text}")
                return None
    ...
    async def verify_patient_exists(self, patient_id, auth_header=None):
        ...
        try:
            ...
            logger_service.error(f"Error verifying patient existence: {e!s}")
            return False

    async def get_patient_medical_history(self, patient_id, auth_header=None):
        """Get patient medical history"""
        try:
            return await self.circuit_breaker.call(self._get_patient_medical_history, patient_id, auth_header)
        except Exception as e:
            logger_service.error(f"Error fetching patient medical history: {e!s}")
            return {}
    ...
    async def notify_patient_appointment(self, patient_id, appointment_data, auth_header=None):
        ...
        try:
            ...
            logger_service.error(f"Error notifying patient of appointment: {e!s}")
            return False

    async def _notify_patient_appointment(self, patient_id, appointment_data, auth_header=None):

...
class MedecinService:
    ...
    def __init__(self, config):
        ...
        self.base_url = (
            f"http://{config.MEDECINS_SERVICE_HOST}:{config.MEDECINS_SERVICE_PORT}/api"
        )
    ...
    async def get_doctor_by_id(self, doctor_id, auth_header=None):
        ...
        try:
            ...
            result = await self.circuit_breaker.call(
                self._get_doctor_by_id, doctor_id, auth_header
            )
            return result
        except Exception as e:
            logger_service.error(f"Error fetching doctor with ID {doctor_id}: {str(e)}")
            return None
    ...
    async def _get_doctor_by_id(self, doctor_id, auth_header=None):
        ...
        if auth_header:
            ...
            logger_service.info(
                f"Using auth header for doctor lookup: {auth_header[:20]}..."
            )
        else:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger_service.info(
                    f"Doctor lookup response status: {response.status_code}"
                )
                if response.status_code == 200:
                    ...
                    logger_service.error(
                        f"Error fetching doctor: HTTP {response.status_code} - {response.text}"
                    )
            ...
            logger_service.error(f"Exception in doctor lookup: {str(e)}")
            return None
    ...
    async def get_available_doctors(self, specialty=None, date=None, auth_header=None):
        ...
        try:
            return await self.circuit_breaker.call(
                self._get_available_doctors, specialty, date, auth_header
            )
        except Exception as e:
            logger_service.error(f"Error fetching available doctors: {str(e)}")
            return []
    ...
    async def _get_available_doctors(self, specialty=None, date=None, auth_header=None):
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.get(
                f"{self.base_url}/doctors", params=params, headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                logger_service.error(
                    f"Error fetching available doctors: HTTP {response.status_code} - {response.text}"
                )
                return []
    ...
    async def check_doctor_availability(self, doctor_id, date, auth_header=None):
        ...
        try:
            return await self.circuit_breaker.call(
                self._check_doctor_availability, doctor_id, date, auth_header
            )
        except Exception as e:
            logger_service.error(f"Error checking doctor availability: {str(e)}")
            return False
    ...
    async def notify_doctor_appointment(
            self, doctor_id, appointment_data, auth_header=None
    ):
        ...
        try:
            ...
            logger_service.error(f"Error notifying doctor of appointment: {str(e)}")
            return False

    async def _notify_doctor_appointment(
            self, doctor_id, appointment_data, auth_header=None
    ):

...
async def get_doctor_appointments(
    ...
    if current_user.get("user_id") != doctor_id and "admin" not in current_user.get(
            "roles", []
    ):
        raise HTTPException(
...
async def get_patient_appointments(
    ...
    if current_user.get("user_id") != patient_id and "admin" not in current_user.get(
            "roles", []
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_doctor_patient_appointments(
    ...
    logger_service.info(
        f"Getting appointments between doctor {doctor_id} and patient {patient_id}"
    )

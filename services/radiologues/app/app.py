    ...
    try:
        ...
        for rapport in rapports:
            ...
            rapport_list.append({
                ...
                "date": str(rapport["date"]) if "date" in rapport else None  # Conversion sûre
            })
...
def scrape_medtn_doctors():
    ...
    if response.status_code == 200:
        ...
        for doctor_card in soup.find_all('div', class_='doctor-card'):
            name = doctor_card.find('h2').text.strip()
            specialty = doctor_card.find('p', class_='specialty').text.strip()
            location = doctor_card.find('p', class_='location').text.strip()
            profile_url = doctor_card.find('a', class_='profile-link')['href']
            doctor = {
                'name': name,
                'specialty': specialty,
                'location': location,
...
@app.route('/api/scrape_doctors', methods=['GET'])

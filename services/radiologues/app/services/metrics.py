...
meter = metrics.get_meter("radiologues_service")

# RabbitMQ metrics
RABBITMQ_MESSAGES_PUBLISHED = meter.create_counter(
    name="radiologues_service_rabbitmq_messages_published_total",
    description="Total number of messages published by radiologues service",
)

RABBITMQ_PUBLISH_LATENCY = meter.create_histogram(
    name="radiologues_service_rabbitmq_publish_latency_seconds",
    description="Message publish latency in seconds for radiologues service",
)

RABBITMQ_CONSUME_LATENCY = meter.create_histogram(
    name="radiologues_service_rabbitmq_consume_latency_seconds",
    description="Message consume latency in seconds for radiologues service",
)

RABBITMQ_QUEUE_SIZE = meter.create_up_down_counter(
    name="radiologues_service_rabbitmq_queue_size",
    description="Number of messages in queue for radiologues service",
)

RABBITMQ_CONSUMER_COUNT = meter.create_up_down_counter(
    name="radiologues_service_rabbitmq_consumers",
    description="Number of consumers for radiologues service",
)

# Circuit breaker metrics
CIRCUIT_BREAKER_STATE = meter.create_up_down_counter(
    name="radiologues_service_circuit_breaker_state",
    description="Circuit breaker state (0=closed, 1=open, 2=half-open) for radiologues service",
)

CIRCUIT_BREAKER_FAILURES = meter.create_counter(
    name="radiologues_service_circuit_breaker_failures_total",
    description="Number of circuit breaker failures for radiologues service",
)

CIRCUIT_BREAKER_SUCCESS = meter.create_counter(
    name="radiologues_service_circuit_breaker_success_total",
    description="Number of circuit breaker successes for radiologues service",
)

CIRCUIT_BREAKER_REJECTED = meter.create_counter(
    name="radiologues_service_circuit_breaker_rejected_total",
    description="Number of requests rejected due to open circuit for radiologues service",
)

# Cache metrics
CACHE_HITS = meter.create_counter(
    name="radiologues_service_cache_hits_total",
    description="Number of cache hits for radiologues service",
)

CACHE_MISSES = meter.create_counter(
    name="radiologues_service_cache_misses_total",
    description="Number of cache misses for radiologues service",
)

# Service dependency metrics
SERVICE_DEPENDENCY_UP = meter.create_up_down_counter(
    name="radiologues_service_dependency_up",
    description="Whether a service dependency is available (1=up, 0=down) for radiologues service",
)

SERVICE_DEPENDENCY_LATENCY = meter.create_histogram(
    name="radiologues_service_dependency_latency_seconds",
    description="Service dependency request latency in seconds for radiologues service",
)

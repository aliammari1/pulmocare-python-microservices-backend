...
class PrometheusService:
    ...
    def __init__(self, app, config):
        ...
        self.metrics.info('reports_service_info', 'Reports service info', version='1.0.0')
    ...
    def start_metrics_server(self):
        ...
        try:
            ...
            if e.errno == 98:  # Address already in use
                ...
                self.logger.warning(f"Port {self.config.METRICS_PORT} is already in use. Trying fallback port {fallback_port}")
                try:
                    start_http_server(fallback_port)
                    self.logger.info(f"Prometheus metrics server started on fallback port {fallback_port}")
                except Exception as fallback_error:
                    self.logger.error(f"Failed to start Prometheus metrics server on fallback port: {str(fallback_error)}")
            else:
                self.logger.error(f"Failed to start Prometheus metrics server: {str(e)}")

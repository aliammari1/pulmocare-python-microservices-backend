        ...
        def health_check():
            ...
            try:
                mongo_client = MongoClient(
                    config.get_mongodb_uri(), serverSelectionTimeoutMS=2000
                )
                mongo_client.admin.command("ping")
                health["checks"]["mongodb"] = {"status": "up"}
            except Exception as e:
                error_msg = str(e)
                health["checks"]["mongodb"] = {"status": "down", "error": error_msg}
                health["status"] = "degraded"

            # Check Redis connection
            try:
                redis_client = redis.from_url(
                    config.REDIS_URL, socket_connect_timeout=2
                )
                redis_client.ping()
                health["checks"]["redis"] = {"status": "up"}
            except Exception as e:
                error_msg = str(e)
                health["checks"]["redis"] = {"status": "down", "error": error_msg}
                health["status"] = "degraded"
            ...
            try:
                ...
                error_msg = str(e)
                health["checks"]["system"] = {"status": "unknown", "error": error_msg}

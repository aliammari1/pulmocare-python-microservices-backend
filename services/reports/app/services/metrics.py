...
meter = metrics.get_meter("reports_service")

# RabbitMQ metrics
RABBITMQ_MESSAGES_PUBLISHED = meter.create_counter(
    name="reports_service_rabbitmq_messages_published_total",
    description="Total number of messages published by reports service",
)

RABBITMQ_PUBLISH_LATENCY = meter.create_histogram(
    name="reports_service_rabbitmq_publish_latency_seconds",
    description="Message publish latency in seconds for reports service",
)

RABBITMQ_CONSUME_LATENCY = meter.create_histogram(
    name="reports_service_rabbitmq_consume_latency_seconds",
    description="Message consume latency in seconds for reports service",
)

RABBITMQ_QUEUE_SIZE = meter.create_up_down_counter(
    name="reports_service_rabbitmq_queue_size",
    description="Number of messages in queue for reports service",
)

RABBITMQ_CONSUMER_COUNT = meter.create_up_down_counter(
    name="reports_service_rabbitmq_consumers",
    description="Number of consumers for reports service",
)

# Circuit breaker metrics
CIRCUIT_BREAKER_STATE = meter.create_up_down_counter(
    name="reports_service_circuit_breaker_state",
    description="Circuit breaker state (0=closed, 1=open, 2=half-open) for reports service",
)

CIRCUIT_BREAKER_FAILURES = meter.create_counter(
    name="reports_service_circuit_breaker_failures_total",
    description="Number of circuit breaker failures for reports service",
)

CIRCUIT_BREAKER_SUCCESS = meter.create_counter(
    name="reports_service_circuit_breaker_success_total",
    description="Number of circuit breaker successes for reports service",
)

CIRCUIT_BREAKER_REJECTED = meter.create_counter(
    name="reports_service_circuit_breaker_rejected_total",
    description="Number of requests rejected due to open circuit for reports service",
)

# Cache metrics
CACHE_HITS = meter.create_counter(
    name="reports_service_cache_hits_total",
    description="Number of cache hits for reports service",
)

CACHE_MISSES = meter.create_counter(
    name="reports_service_cache_misses_total",
    description="Number of cache misses for reports service",
)

# Service dependency metrics
SERVICE_DEPENDENCY_UP = meter.create_up_down_counter(
    name="reports_service_dependency_up",
    description="Whether a service dependency is available (1=up, 0=down) for reports service",
)

SERVICE_DEPENDENCY_LATENCY = meter.create_histogram(
    name="reports_service_dependency_latency_seconds",
    description="Service dependency request latency in seconds for reports service",
)
